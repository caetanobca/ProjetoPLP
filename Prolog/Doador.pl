...
getDoadorEndereco(doador(_,Endereco,_,_,_,_,_,_), Endereco).
getDoadorIdade(doador(_,_,Idade,_,_,_,_,_), Idade).
getDoadorTelefone(doador(_,_,_,Telefone,_,_,_,_), Telefone).
getDoadorTipSanguineo(doador(_,_,_,_,TipSanguineo,_,_,_), TipSanguineo).
getDoadorImpedimentoStr(doador(_,_,_,_,_,ImpedimentoStr,_,_), ImpedimentoStr).
getDoadorUltimoDiaImpedido(doador(_,_,_,_,_,_,UltimoDiaImpedido,_), UltimoDiaImpedido).
getDoadorDoacoes(doador(_,_,_,_,_,_,_,Doacoes), Doacoes).
...
doadoresToString(doador(Nome,Endereco,Idade,Telefone), Result):- string_concat("Nome: ", Nome, Parte1), string_concat(Parte1, " Endereço: ", Parte2),
                    ...
                    string_concat(Parte11, " Ultimo Dia Impedido: ", Parte12), string_concat(Parte12, UltimoDiaImpedido, Parte13), 
                    string_concat(Parte13, " Doacoes: ", Parte14), string_concat(Parte14, Doacoes, Result).
